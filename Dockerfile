...
# create nginx user/group first, to be consistent throughout docker variants
    && addgroup --system --gid 82 fbzs \
    && adduser --system --disabled-login --ingroup fbzs --no-create-home --home /nonexistent --gecos "nginx user" --shell /bin/false --uid 82 fbzs \
    && apt-get update \
...
#健康检查
HEALTHCHECK --interval=30s --timeout=30s --retries=3 CMD  curl --fail http://127.0.0.1:80/ || exit 1
EXPOSE 80

#更改缓存目录属主属组
RUN chown -R fbzs.fbzs /var/cache/nginx ; \
    chown -R fbzs.fbzs /var/log/nginx ; \
    chown -R fbzs.fbzs /var/run/ 
STOPSIGNAL SIGQUIT
